...
class ZassPopularPostsWidget extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array(
			'classname'   => 'widget_recent_entries zass-popular-posts',
			'description' => esc_html__( "The most popular posts on your site", 'zass-plugin' )
		);
	...
	function widget( $args, $instance ) {
		...
		$title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : esc_html__('Popular Posts', 'zass-plugin');
                ...
                <li>
                    <a href="<?php the_permalink( $popular_post->ID ); ?>" <?php echo $aria_current; ?>title="<?php echo esc_attr( get_the_title() ? get_the_title() : get_the_ID() ); ?>">
						<?php if ( has_post_thumbnail( $popular_post->ID ) ): ?>
							<?php echo get_the_post_thumbnail( $popular_post->ID, 'zass-widgets-thumb', '' ); ?>
						<?php endif; ?>
						<?php
