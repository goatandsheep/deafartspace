	...
	class YITH_Stripe_Connect_Gateway extends WC_Payment_Gateway_CC {
		...
		public function pay( $order = null ) {
			...
			$this->api_handler->update_intent(
				...
				array(
					...
					'description'    => apply_filters( 'yith_wcsc_charge_description', sprintf( __( '%1$s - Order %2$s', 'yith-stripe-connect-for-woocommerce' ), esc_html( get_bloginfo( 'name' ) ), $order->get_order_number() ), esc_html( get_bloginfo( 'name' ) ), $order->get_order_number() ),
			...
			if ( $this->is_alt_flow( $order->get_id() ) ) {
		...
		public function create_session_intent( $args = array() ) {
			...
			if ( $commissions = $this->is_alt_flow( $order_id ) ) {
				...
				$fee            = max( 0, $total - $commission_amt );
		...
		public function is_direct_charges( $order_id = false ) {
			...
			$is_direct_charges     = $this->is_alt_flow( $order_id ) && 'direct_charges' == $this->alt_flow && ! $this->order_contains_subscription( $order_id );
			$supports_tokenization = array_search( 'tokenization', $this->supports );

			if ( $is_direct_charges && false !== $supports_tokenization ) {
				unset( $this->supports[ $supports_tokenization ] );
			}

			return $is_direct_charges;
		}
