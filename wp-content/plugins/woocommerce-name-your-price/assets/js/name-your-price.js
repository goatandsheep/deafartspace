...
( function( $ ) {
	...
	nypForm.prototype.initIntegrations = function() {
		...
		if ( this.$el.hasClass( 'variations_form' ) || this.$el.find( '.variations_form' ).length ) {
	...
	function WC_NYP_Variations_Integration( form ) {
		...
		var self = this;
		self.elements = form.$el.hasClass( 'variations_form' ) ? form.$el : form.$el.find( '.variations_form' );
		...
		this.integrate = function() {;
			self.elements.on( 'found_variation', self.onFoundVariation );
			self.elements.on( 'reset_image', self.resetVariations );
			self.elements.on( 'click', '.reset_variations', self.resetVariations );
		}

		// When variation is found, decide if it is NYP or not.
		this.onFoundVariation = function( event, variation ) {

			var $nyp = $( event.target ).find( '.nyp' );
			var nyp = $nyp.length ? $nyp.wc_nyp_get_script_object() : false;
