...
function wc_get_gallery_image_html($attachment_id, $main_image = false)
{
	...
	return '<div data-thumb="' . esc_url($thumbnail_src[0]) . '" data-thumb-alt="' . esc_attr($alt_text) . '" class="woocommerce-product-gallery__image"><a title="' . _wp_specialchars(get_post_field('post_title', $attachment_id), ENT_QUOTES, 'UTF-8', true) . '" alt="' . _wp_specialchars(get_post_field('post_title', $attachment_id), ENT_QUOTES, 'UTF-8', true) . '" href="' . esc_url($full_src[0]) . '">' . $image . '</a></div>';
}
